...
function Post() {
  ...
  const checkButonPermission = (post) => {
    if (post?.author?.name == currentUser?.email) {
      return true
    } else return false
  }

  const test = async (post) => {
    console.log('currentUser.email: ', currentUser.email);
    console.log('post.author.name: ', post.author.name);
    console.log('post.author.name == currentUser.email: ', post.author.name == currentUser.email);
    if (post.author.name == currentUser.email) {
      console.log('false')
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className='PostPage'>
      ...
      <Container className='post_content' fluid>
        {dataEachPage.map((post) => (
          <div className='post' key={post.id}>
            <div className='container' fluid>
              <div className='row border-bottom pb-1'>
                ...
                <div className='col d-flex flex-row-reverse'>
                  <div className='row'>
                    <div className='col'>
                      <Button className='button_fix' onClick={() => { handleShow(post.id) }} disabled={!checkButonPermission(post)}>
                        Sửa
                      </Button>
                    </div>
                    <div className='col'>
                      <Button variant='danger' className='button_delete' onClick={() => { deletePost(post.id) }} disabled={!checkButonPermission(post)}>
                        Xóa
                      </Button>
                    </div>
                    {/* <div className='col'>                <button className='button_delete' onClick={() => { test(post) }}>test</button></div> */}
                  </div>
            ...
            <h6 className='text-white ms-1 mt-1 border-bottom pb-1'>Tác giả : {post.author.name}</h6>
